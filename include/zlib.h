...
#define zlib_version zlibVersion()
  ...
  */
  ...
  exact value depends on the compression method), inflateInit determines the
     ...
     inflate.  If zalloc and zfree are set to Z_NULL, inflateInit updates them
  ...
  */
  ...
  /*
    ...
    so far (that is, total_out bytes) and returns Z_OK, Z_STREAM_END or an error
    code as described below. At the end of the stream, inflate() checks that its
    computed adler32 checksum is equal to that saved by the compressor and
    returns Z_STREAM_END only if the checksum is correct.
    ...
    the zlib format or incorrect check value), Z_STREAM_ERROR if the stream
    ...
    Z_DATA_ERROR is returned, the application may then call inflateSync() to
    look for a good compression block if a partial recovery of the data is
    desired.
  */
  ...
  */
  ...
  ZEXTERN int ZEXPORT deflateInit2 OF((z_streamp strm,
     ...
     with no zlib header or trailer, and will not compute an adler32 check
  value.
  ...
  strategy parameter only affects the compression ratio but not the correctness
  of the compressed output even if it is not set appropriately.  Z_FIXED
  prevents the use of dynamic Huffman codes, allowing for a simpler decoder for
  ...
  */
  ...
  not looking for any check values for comparison at the end of the stream. This
     ...
     be done by inflate(). (So next_in and avail_in may be modified, but
  ...
  */
  ...
  /*
     ...
     returned an error.  If strm->next is not Z_NULL, then the Z_BUF_ERROR was
     due to out() returning non-zero.  (in() will always be called before out(),
     so strm->next_in is assured to be defined if out() returns non-zero.)  Note
  ...
  */
  ...
  /*
     ...
     return return an error (0) with nothing written. In this case, there may
     also be a buffer overflow with unpredictable consequences, which is
     possible only if zlib was compiled with the insecure functions sprintf() or
     vsprintf() because the secure snprintf() or vsnprintf() functions were not
     available.
  */
