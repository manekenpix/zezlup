...
#define GLFW_DONT_CARE -1
   ...
   *
   *  This is the function pointer type for window size callbacks.  A window
   * size callback function has the following signature:
   *  @code
   ...
   *  @ingroup window
   */
  typedef void ( *GLFWwindowsizefun )( GLFWwindow* window,
                                       int width,
                                       int height );
   ...
   *  application locale according to the current environment if that category
   * is still "C".  This is because the "C" locale breaks Unicode text input.
   *
   ...
   * to call this function, as it is called by @ref glfwInit before it returns
   *  failure.
   *
   ...
   * them. Setting these hints requires no platform specific headers or
   * functions.
   *
   ...
   *  library.  It is intended for when you are using GLFW as a shared library
   ...
   *  glfwGetError from the error callback will return the same value as the
   * error code argument.
   *
   ...
   * If the pixel dimensions of your UI scaled by this look appropriate on your
   ...
   * by color bit depth (the sum of all channel depths), then by resolution area
   * (the product of width and height), then resolution width and finally by
   * refresh rate.
   *
   *  @param[in] monitor The monitor to query.
   ...
   *  freed by GLFW.  You should not free them yourself.  They are valid until
   * the specified monitor is disconnected, this function is called again for
   ...
   *  function or @ref glfwDefaultWindowHints, or until the library is
   * terminated.
   *
   ...
   *  If you set hints to invalid values this will instead be reported by the
   * next call to @ref glfwCreateWindow.
   *
   *  Some hints are platform specific.  These may be set on any platform but
   * they will only affect their specific platform.  Other platforms will ignore
   * them. Setting these hints requires no platform specific headers or
   ...
   *  recommended that you pick the primary monitor.  For more information on
   * how to query connected monitors, see @ref monitor_monitors.
   *
   ...
   *  initially invisible using the [GLFW_VISIBLE](@ref GLFW_VISIBLE_hint)
   ...
   * window dimensions may be overridden by the window system on creation. Check
   * the actual [size](@ref window_size) after creation.
   *
   ...
   * present, the `IDI_APPLICATION` icon will be used instead.  To set a
   * different icon, see @ref glfwSetWindowIcon.
   *
   *  @remark @win32 The context to share resources with must not be current on
   *  any other thread.
   *
   *  @remark @macos The OS only supports forward-compatible core profile
   * contexts for OpenGL versions 3.2 and later.  Before creating an OpenGL
   * context of version 3.2 or later you must set the
   ...
   *  bar.  Otherwise a minimal menu bar is created manually with common
   * commands like Hide, Quit and About.  The About entry opens a minimal about
   * dialog with information from the application's bundle.  Menu bar creation
   * can be disabled entirely with the @ref GLFW_COCOA_MENUBAR init hint.
   *
   ...
   * the application bundle's `Info.plist`.  For more information, see [High
   * Resolution Guidelines for OS
   ...
   * for a window to reach its requested state.  This means you may not be able
   * to query the final size, position or other attributes directly after window
   *  creation.
   *
   ...
   * of candidate images, those of or closest to the sizes desired by the system
   ...
   * file. This function always emits @ref GLFW_PLATFORM_ERROR.
   *
   *  @thread_safety This function must only be called from the main thread.
   *
   *  @sa @ref window_icon
   ...
   *
   *  @param[in] window The window to query.
   *  @param[out] xpos Where to store the x-coordinate of the upper-left corner
   * of the content area, or `NULL`.
   *  @param[out] ypos Where to store the y-coordinate of the upper-left corner
   * of the content area, or `NULL`.
   *
   *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED and @ref
   *  GLFW_PLATFORM_ERROR.
   ...
   *  one, where zero is fully transparent and one is fully opaque.  If the
   ...
   * original video mode of the monitor.  The window's desired video mode is set
   ...
   *  (minimized) or maximized.  If the window is already restored, this
   ...
   *  GLFW_PLATFORM_ERROR.
   *
   *  @remark @wayland It is not possible for an application to bring its
   * windows to front, this function will always emit @ref GLFW_PLATFORM_ERROR.
   *
   *  @thread_safety This function must only be called from the main thread.
   *
   *  @sa @ref window_focus
   ...
   *  GLFW_PLATFORM_ERROR.
   *
   *  @remark @macos Attention is requested to the application as a whole, not
   * the specific window.
   *
   *  @thread_safety This function must only be called from the main thread.
   *
   ...
   *  affected by any resizing or mode switching, although you may need to
   * update your viewport if the framebuffer size has changed.
   *
   ...
   *
   *  @param[in] window The window to query.
   *  @param[in] attrib The [window attribute](@ref window_attribs) whose value
   * to return.
   ...
   *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED.
   *
   *  @remark @wayland This callback will never be called, as there is no way
   ...
   * example if the window has been exposed after having been covered by another
   * window.
   *
   ...
   * on those platforms.  You can use the [window refresh callback](@ref
   * window_refresh) to redraw the contents of your window when necessary during
   * such operations.
   *
   *  Do not assume that callbacks you set will _only_ be called in response to
   *  event processing functions like this one.  While it is necessary to poll
   * for events, window systems that require GLFW to register callbacks of its
   * own can pass events to GLFW in response to many window system function
   * calls. GLFW will pass those events on to the application callbacks before
   *  returning.
   *
   ...
   *  are processed and the function then returns immediately.  Processing
   ...
   * return without a callback having been called even if you are monitoring all
   *  callbacks.
   *
   *  On some platforms, a window move, resize or menu operation will cause
   * event processing to block.  This is due to how event processing is designed
   ...
   *
   *  This function puts the calling thread to sleep until at least one event is
   *  available in the event queue, or until the specified timeout is reached.
   * If one or more events are available, it behaves exactly like @ref
   *  glfwPollEvents, i.e. the events in the queue are processed and the
   ...
   * window. The mode must be one of @ref GLFW_CURSOR, @ref GLFW_STICKY_KEYS,
   ...
   * even if the mouse button had been released before the call.  This is useful
   * when you are only interested in whether mouse buttons have been pressed but
   * not when or in which order.
   *
   *  If the mode is `GLFW_LOCK_KEY_MODS`, the value must be either `GLFW_TRUE`
   * to enable lock key modifier bits, or `GLFW_FALSE` to disable them.  If
   * enabled, callbacks that receive modifier bits will also have the @ref
   *  GLFW_MOD_CAPS_LOCK bit set when the event was generated with Caps Lock on,
   *  and the @ref GLFW_MOD_NUM_LOCK bit when Num Lock was on.
   *
   *  If the mode is `GLFW_RAW_MOUSE_MOTION`, the value must be either
   * `GLFW_TRUE` to enable raw (unscaled and unaccelerated) mouse motion when
   * the cursor is disabled, or `GLFW_FALSE` to disable it.  If raw motion is
   * not supported, attempting to set this will emit @ref GLFW_PLATFORM_ERROR.
   * Call @ref glfwRawMouseMotionSupported to check for support.
   *
   ...
   *  system.  This status does not change after GLFW has been initialized so
   * you only need to check this once.  If you attempt to enable raw motion on
   *  a system that does not support it, @ref GLFW_PLATFORM_ERROR will be
   * emitted.
   *
   *  Raw mouse motion is closer to the actual motion of the mouse across
   *  a surface.  It is not affected by the scaling and acceleration applied to
   *  the motion of the desktop cursor.  That processing is suitable for a
   * cursor while raw motion is better for controlling for example a 3D camera.
   ...
   * key, otherwise the scancode is ignored.  If you specify a non-printable
   * key, or `GLFW_KEY_UNKNOWN` and a scancode that maps to a non-printable key,
   ...
   * button to the specified window.  The returned state is one of `GLFW_PRESS`
   * or `GLFW_RELEASE`.
   *
   *  If the @ref GLFW_STICKY_MOUSE_BUTTONS input mode is enabled, this function
   *  returns `GLFW_PRESS` the first time you call it for a mouse button that
   * was pressed, even if that mouse button has already been released.
   *
   *  @param[in] window The desired window.
   ...
   * to top edge of the content area, or `NULL`.
   *
   *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED and @ref
   *  GLFW_PLATFORM_ERROR.
   *
   ...
   * GLFW already provides the `GLFW_CURSOR_DISABLED` cursor mode that hides the
   ...
   * corner of the cursor image.  Like all other coordinate systems in GLFW, the
   * X-axis points to the right and the Y-axis points down.
   *
   ...
   * after the focus loss event has been processed, i.e. after the [window focus
   * callback](@ref glfwSetWindowFocusCallback) has been called.
   *
   ...
   *  not be called if modifier keys are held down that would prevent normal
   * text input on that platform, for example a Super (Command) key on macOS or
   * Alt key on Windows.
   *
   *  @param[in] window The window whose callback to set.
   *  @param[in] callback The new callback, or `NULL` to remove the currently
   ...
   *  callback, the character with modifiers callback deals with characters and
   * is keyboard layout dependent.  Characters do not map 1:1 to physical keys,
   * as a key may produce zero, one or more characters.  If you want to know
   * whether a specific physical key was pressed or released, see the [key
   * callback](@ref glfwSetKeyCallback) instead.
   *
   *  @param[in] window The window whose callback to set.
   *  @param[in] callback The new callback, or `NULL` to remove the currently
   * set callback.
   ...
   *  window, which is called when the cursor enters or leaves the content area
   * of the window.
   *
   *  @param[in] window The window whose callback to set.
   *  @param[in] callback The new callback, or `NULL` to remove the currently
   ...
   *
   *  This function sets the path drop callback of the specified window, which
   ...
   * specifically for that event, they are not guaranteed to be valid after the
   * callback has returned.  If you wish to use them after the callback returns,
   ...
   *  represented as four buttons.  The hats are in the same order as returned
   * by
   ...
   * returned array.  This is set to zero if the joystick is not present or an
   * error occurred.
   ...
   * right, down and left) directions and you can test for these individually by
   ...
   *  @pointer_lifetime The returned array is allocated and freed by GLFW.  You
   *  should not free it yourself.  It is valid until the specified joystick is
   *  disconnected, this function is called again for that joystick or the
   * library is terminated.
   *
   *  @thread_safety This function must only be called from the main thread.
   *
   ...
   * library is terminated and re-initialized the internal list will revert to
   ...
   * mapping this function will return `NULL` but will not generate an error.
   * Call
   ...
   *
   *  If the specified joystick is not present or does not have a gamepad
   * mapping this function will return `GLFW_FALSE` but will not generate an
   * error.  Call
   *  @ref glfwJoystickPresent to check whether it is present regardless of
   *  whether it has a mapping.
   *
   *  The Guide button may not be available for input as it is often hooked by
   * the system or the Steam client.
   *
   ...
   *  has been set using @ref glfwSetTime it measures time elapsed since GLFW
   * was initialized.
   *
   ...
   * window current on the calling thread.  A context must only be made current
   * on a single thread at a time and each thread can have only a single current
   *  context at a time.
   *
   ...
   *  because of a user setting that overrides the application's request or due
   ...
   * glfwVulkanSupported to check whether Vulkan is at least minimally available
   * and @ref glfwGetRequiredInstanceExtensions to check what instance
   ...
   * found, this function returns `VK_ERROR_INITIALIZATION_FAILED` and generates
   * a @ref GLFW_API_UNAVAILABLE error.  Call @ref glfwVulkanSupported to check
   ...
   * extensions enabled, this function returns `VK_ERROR_EXTENSION_NOT_PRESENT`
   * and generates a @ref GLFW_API_UNAVAILABLE error.  Call @ref
   ...
   *  set to `GLFW_NO_API` otherwise it generates a @ref GLFW_INVALID_VALUE
   ...
   *  glfwGetRequiredInstanceExtensions.
   *
   *  @remark @macos This function creates and sets a `CAMetalLayer` instance
