...
axisAngle( mat<4, 4, T, Q> const& m, vec<3, T, Q>& axis, T& angle )
{
  ...
  T const angleCos = ( m[0][0] + m[1][1] + m[2][2] - static_cast<T>( 1 ) ) *
                     static_cast<T>( 0.5 );
...
axisAngleMatrix( vec<3, T, Q> const& axis, T const angle )
{
  T c = cos( angle );
  T s = sin( angle );
...
template<typename T, qualifier Q>
GLM_FUNC_QUALIFIER mat<4, 4, T, Q>
interpolate( mat<4, 4, T, Q> const& m1,
